...
const Select = ({ value, onChange, children }) => (
  <select
    ...
    className="h-10 w-full rounded-auto-md border border-auto-border bg-auto-surface px-3 text-sm text-auto-text outline-none focus:ring-2 focus:ring-auto-accent/40"
  >
    {children}
  </select>
);
...
export default function TicketsSidebarAutos({
  ...
  const visibleTickets = useMemo(() => {
    ...
    return tickets.filter((t) => String(t?.contact?.leadSource || "").toUpperCase() === target);
  }, [tickets, filters.leadSource]);

  return (
    <div className="flex h-full min-h-0 flex-col rounded-auto-xl border border-auto-border bg-auto-panel shadow-auto-soft">
      <div className="border-b border-auto-border px-4 py-4">
        ...
        {statusTabs.map((t) => {
          ...
          return (
            <button
              ...
              className={clsx(
                "h-10 rounded-auto-md px-2 text-xs font-medium transition",
                isActive
                  ? "bg-auto-accent text-white shadow-sm"
                  : "bg-auto-surface text-auto-muted hover:bg-auto-panel2"
              )}
              type="button"
            >
      ...
      <div className="border-b border-auto-border p-3">
        <input
          value={filters.search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Buscar por nombre, teléfono o mensaje…"
          className="h-10 w-full rounded-auto-md border border-auto-border bg-auto-surface px-3 text-sm text-auto-text outline-none placeholder:text-auto-muted focus:ring-2 focus:ring-auto-accent/40"
        />
        ...
        <div className="mt-2 flex items-start justify-between gap-2">
          ...
          <button
            ...
            className="h-8 shrink-0 rounded-auto-md border border-auto-border bg-auto-surface px-3 text-xs font-medium text-auto-text hover:bg-auto-panel2"
            title="Filtros"
          >
        ...
        {loading ? (
          <div className="space-y-2 p-2">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-16 animate-pulse rounded-auto-lg border border-auto-border bg-auto-panel2" />
            ))}
          </div>
        ) : visibleTickets.length === 0 ? (
          <div className="p-6 text-center text-sm text-auto-muted">No hay tickets en esta vista.</div>
        ) : (
