...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async connectionUpdate({ qr, connection, lastDisconnect }: Partial<ConnectionState>) {
    ...
    if (connection === 'close') {
      ...
      const codesToNotReconnect = [DisconnectReason.loggedOut, DisconnectReason.forbidden, 402, 406, 440]; // 440 = connectionReplaced — no reconectar si otra sesión tomó el control
      ...
      try {
        for (const received of messages) {
          ...
          if (this.configService.get<Database>('DATABASE').SAVE_DATA.NEW_MESSAGE) {
            ...
            // Ensure a Chat row exists for this remoteJid.
            // The Manager's "Chats" list is built from DB state; if HISTORIC sync was disabled
            // (or didn't run), you can end up with messages but no Chat rows.
            // Upserting here makes chat visibility resilient.
            if (this.configService.get<Database>('DATABASE').SAVE_DATA.CHATS) {
